...
const AUTO_DEPOSIT = gql`
  mutation($month: String!) {
    autoDepositSaMonth(month: $month)
  }
`;

const REMOVE_MONTH = gql`
  mutation($month: String!) {
    removeSaMonth(month: $month)
  }
`;

export class SaMonthCollections extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  handleSelect(eventKey) {
    ...
    this.props.history.push(`/sa-month-collections/${eventKey}`);
  }
              ...
              <thead>
                <tr>
                  <th colSpan="10" className="text-center h4">
                    <strong>Security A/c Monthly Collections</strong>
                    &nbsp;
                  </th>
                </tr>
                <tr>
                  ...
                  <tr key={detail.DepositDate}>
                    ...
                    <td style={Styles.PaddingThreeCenter}>
                      <Link
                        ...
                        to={`/sa-month-collections-print/${detail._id}`}
                      >
...
const FormatData = props => {
  ...
  return (
    <SaMonthCollections
      ...
      monthTotals={props.saMonthTotalsByPage.monthTotals}
      count={props.saMonthTotalsByPage.count}
      match={props.match}
...
FormatData.propTypes = {
  ...
  saMonthTotalsByPage: PropTypes.object.isRequired,
  history: PropTypes.object.isRequired,
  match: PropTypes.object.isRequired,
  refetch: PropTypes.func.isRequired,
  client: PropTypes.instanceOf(ApolloClient)
...
const SA_MONTH_COLLECTIONS = gql`
  query($pageNo: Int!) {
    saMonthTotalsByPage(pageNo: $pageNo)
  }
`;

export default graphql(SA_MONTH_COLLECTIONS, {
  props: ({ data: { loading, saMonthTotalsByPage, refetch } }) => ({
    loading,
    saMonthTotalsByPage,
    refetch
  }),
